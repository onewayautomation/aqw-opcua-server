...
@param const weatherserver::WeatherData& weatherData - The WeatherData object the data will be requested from.
@param std::string weatherVariableName - The weather browse name to check which variable node to update.
*/
static void updateWeatherVariables(UA_DataValue *dataValue, const weatherserver::WeatherData& weatherData, std::string weatherVariableName) {
    if (weatherVariableName == weatherserver::WeatherData::BROWSE_LATITUDE) {
    ...
    } else if (weatherVariableName == weatherserver::WeatherData::BROWSE_LONGITUDE) {
        ...
        dataValue->hasValue = true;
    } else if (weatherVariableName == weatherserver::WeatherData::BROWSE_TIMEZONE) {
        ...
        dataValue->hasValue = true;
    } else if (weatherVariableName == weatherserver::WeatherData::BROWSE_ICON) {
        ...
        dataValue->hasValue = true;
    } else if (weatherVariableName == weatherserver::WeatherData::BROWSE_TEMPERATURE) {
        ...
        dataValue->hasValue = true;
    } else if (weatherVariableName == weatherserver::WeatherData::BROWSE_APPARENT_TEMPERATURE) {
        ...
        dataValue->hasValue = true;
    } else if (weatherVariableName == weatherserver::WeatherData::BROWSE_HUMIDITY) {
        ...
        dataValue->hasValue = true;
    } else if (weatherVariableName == weatherserver::WeatherData::BROWSE_PRESSURE) {
  ...
  else if (weatherVariableName == weatherserver::WeatherData::BROWSE_WIND_SPEED) {
  ...
  else if (weatherVariableName == weatherserver::WeatherData::BROWSE_WIND_BEARING) {
    ...
    dataValue->hasValue = true;
    } else if (weatherVariableName == weatherserver::WeatherData::BROWSE_CLOUD_COVER) {
...
static UA_StatusCode readRequest(UA_Server *server, const UA_NodeId *sessionId, void *sessionContext,
    ...
    if (nodeId->identifierType == UA_NODEIDTYPE_STRING && nodeId->namespaceIndex == weatherserver::WebService::OPC_NS_INDEX) {
        ...
        */
        ...
        auto searchCountry = weatherserver::CountryData {countryCode};
        ...
        */
        ...
        auto searchLocation = weatherserver::LocationData {locationName, countryCode};
        ...
        while (itLocation == itCountry->getLocations().end()) {
            ...
            // Search for the location in the list of locations inside the country of the web service.
            searchLocation = weatherserver::LocationData {locationName, countryCode};
        ...
        /* The weather data will be downloaded only for the first time or after a interval of minutes specified by the constant weatherserver::WebService::INTERVAL_DOWNLOAD_WEATHER_DATA. */
        if (!(itLocation->getHasBeenReceivedWeatherData()) || intervalBetweenDownloads.count() >= webService->getSettings().getIntervalDownloadWeatherData()) {
            try {
                webService->fetchWeather(itLocation->getLatitude(), itLocation->getLongitude()).then([&](web::json::value response) {
                    itLocation->setWeatherData(weatherserver::WeatherData::parseJson(response));
...
*/
static void requestWeather(UA_Server* server, weatherserver::LocationData& location, UA_NodeId parentNodeId) {
    ...
    std::string parentNameId = static_cast<std::string>(weatherserver::CountryData::COUNTRIES_FOLDER_NODE_ID)
        + "." + location.getCountryCode() + "." + location.getName();
    std::string latitudeNameId = parentNameId + "." + weatherserver::WeatherData::BROWSE_LATITUDE;
    UA_NodeId latitudeVarNodeId = UA_NODEID_STRING_ALLOC(weatherserver::WebService::OPC_NS_INDEX,
    ...
    latitudeVarAttr.displayName = UA_LOCALIZEDTEXT(locale, weatherserver::WeatherData::BROWSE_LATITUDE);
    ...
    UA_Server_addDataSourceVariableNode(server, latitudeVarNodeId, parentNodeId,
        ...
        UA_QUALIFIEDNAME(weatherserver::WebService::OPC_NS_INDEX, weatherserver::WeatherData::BROWSE_LATITUDE),
        UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE), latitudeVarAttr, latitudeVarDataSource, NULL, NULL);

    // #################### Longitude variable node
    /* Creates the identifier for the node id of the new variable node class
    The identifier for the node id of every variable will be: Countries.CountryCode.LocationName.Variable */
    std::string longitudeNameId = parentNameId + "." + weatherserver::WeatherData::BROWSE_LONGITUDE;
    UA_NodeId longitudeVarNodeId = UA_NODEID_STRING_ALLOC(weatherserver::WebService::OPC_NS_INDEX,
    ...
    longitudeVarAttr.displayName = UA_LOCALIZEDTEXT(locale, weatherserver::WeatherData::BROWSE_LONGITUDE);
    ...
    UA_Server_addDataSourceVariableNode(server, longitudeVarNodeId, parentNodeId,
        UA_NODEID_NUMERIC(0, UA_NS0ID_HASCOMPONENT),
        UA_QUALIFIEDNAME(weatherserver::WebService::OPC_NS_INDEX, weatherserver::WeatherData::BROWSE_LONGITUDE),
    ...
    std::string timezoneNameId = parentNameId + "." + weatherserver::WeatherData::BROWSE_TIMEZONE;
    UA_NodeId timezoneVarNodeId = UA_NODEID_STRING_ALLOC(weatherserver::WebService::OPC_NS_INDEX,
    ...
    timezoneVarAttr.displayName = UA_LOCALIZEDTEXT(locale, weatherserver::WeatherData::BROWSE_TIMEZONE);
    ...
    UA_Server_addDataSourceVariableNode(server, timezoneVarNodeId, parentNodeId,
        UA_NODEID_NUMERIC(0, UA_NS0ID_HASCOMPONENT),
        UA_QUALIFIEDNAME(weatherserver::WebService::OPC_NS_INDEX, weatherserver::WeatherData::BROWSE_TIMEZONE),
        UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE), timezoneVarAttr, timezoneVarDataSource, NULL, NULL);

    // #################### Icon variable node
    std::string iconNameId = parentNameId + "." + weatherserver::WeatherData::BROWSE_ICON;
    UA_NodeId iconVarNodeId = UA_NODEID_STRING_ALLOC(weatherserver::WebService::OPC_NS_INDEX,
        iconNameId.c_str());
    ...
    iconVarAttr.displayName = UA_LOCALIZEDTEXT(locale, weatherserver::WeatherData::BROWSE_ICON);
    ...
    UA_Server_addDataSourceVariableNode(server, iconVarNodeId, parentNodeId,
        UA_NODEID_NUMERIC(0, UA_NS0ID_HASCOMPONENT),
        UA_QUALIFIEDNAME(weatherserver::WebService::OPC_NS_INDEX, weatherserver::WeatherData::BROWSE_ICON),
        UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE), iconVarAttr, iconVarDataSource, NULL, NULL);

    // #################### Temperature variable node
    std::string temperatureNameId = parentNameId + "." + weatherserver::WeatherData::BROWSE_TEMPERATURE;
    UA_NodeId temperatureVarNodeId = UA_NODEID_STRING_ALLOC(weatherserver::WebService::OPC_NS_INDEX,
    ...
    temperatureVarAttr.displayName = UA_LOCALIZEDTEXT(locale, weatherserver::WeatherData::BROWSE_TEMPERATURE);
    ...
    UA_Server_addDataSourceVariableNode(server, temperatureVarNodeId, parentNodeId,
        UA_NODEID_NUMERIC(0, UA_NS0ID_HASCOMPONENT),
        UA_QUALIFIEDNAME(weatherserver::WebService::OPC_NS_INDEX, weatherserver::WeatherData::BROWSE_TEMPERATURE),
    ...
    std::string apparentTemperatureNameId = parentNameId + "." + weatherserver::WeatherData::BROWSE_APPARENT_TEMPERATURE;
    UA_NodeId apparentTemperatureVarNodeId = UA_NODEID_STRING_ALLOC(weatherserver::WebService::OPC_NS_INDEX,
    ...
    apparentTemperatureVarAttr.displayName = UA_LOCALIZEDTEXT(locale, weatherserver::WeatherData::BROWSE_APPARENT_TEMPERATURE);
    ...
    UA_Server_addDataSourceVariableNode(server, apparentTemperatureVarNodeId, parentNodeId,
        UA_NODEID_NUMERIC(0, UA_NS0ID_HASCOMPONENT),
        UA_QUALIFIEDNAME(weatherserver::WebService::OPC_NS_INDEX, weatherserver::WeatherData::BROWSE_APPARENT_TEMPERATURE),
    ...
    std::string humidityNameId = parentNameId + "." + weatherserver::WeatherData::BROWSE_HUMIDITY;
    UA_NodeId humidityVarNodeId = UA_NODEID_STRING_ALLOC(weatherserver::WebService::OPC_NS_INDEX,
    ...
    humidityVarAttr.displayName = UA_LOCALIZEDTEXT(locale, weatherserver::WeatherData::BROWSE_HUMIDITY);
    ...
    UA_Server_addDataSourceVariableNode(server, humidityVarNodeId, parentNodeId,
        UA_NODEID_NUMERIC(0, UA_NS0ID_HASCOMPONENT),
        UA_QUALIFIEDNAME(weatherserver::WebService::OPC_NS_INDEX, weatherserver::WeatherData::BROWSE_HUMIDITY),
        UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE), humidityVarAttr, humidityVarDataSource, NULL, NULL);

    // #################### Pressure variable node
    std::string pressureNameId = parentNameId + "." + weatherserver::WeatherData::BROWSE_PRESSURE;
    UA_NodeId pressureVarNodeId = UA_NODEID_STRING_ALLOC(weatherserver::WebService::OPC_NS_INDEX,
    ...
    pressureVarAttr.displayName = UA_LOCALIZEDTEXT(locale, weatherserver::WeatherData::BROWSE_PRESSURE);
    ...
    UA_Server_addDataSourceVariableNode(server, pressureVarNodeId, parentNodeId,
        UA_NODEID_NUMERIC(0, UA_NS0ID_HASCOMPONENT),
        UA_QUALIFIEDNAME(weatherserver::WebService::OPC_NS_INDEX, weatherserver::WeatherData::BROWSE_PRESSURE),
    ...
    std::string windSpeedNameId = parentNameId + "." + weatherserver::WeatherData::BROWSE_WIND_SPEED;
    UA_NodeId windSpeedVarNodeId = UA_NODEID_STRING_ALLOC(weatherserver::WebService::OPC_NS_INDEX,
    ...
    windSpeedVarAttr.displayName = UA_LOCALIZEDTEXT(locale, weatherserver::WeatherData::BROWSE_WIND_SPEED);
    ...
    UA_Server_addDataSourceVariableNode(server, windSpeedVarNodeId, parentNodeId,
        UA_NODEID_NUMERIC(0, UA_NS0ID_HASCOMPONENT),
        UA_QUALIFIEDNAME(weatherserver::WebService::OPC_NS_INDEX, weatherserver::WeatherData::BROWSE_WIND_SPEED),
    ...
    std::string windBearingNameId = parentNameId + "." + weatherserver::WeatherData::BROWSE_WIND_BEARING;
    UA_NodeId windBearingVarNodeId = UA_NODEID_STRING_ALLOC(weatherserver::WebService::OPC_NS_INDEX,
    ...
    windBearingVarAttr.displayName = UA_LOCALIZEDTEXT(locale, weatherserver::WeatherData::BROWSE_WIND_BEARING);
    ...
    UA_NODEID_NUMERIC(0, UA_NS0ID_HASCOMPONENT),
    UA_QUALIFIEDNAME(weatherserver::WebService::OPC_NS_INDEX, weatherserver::WeatherData::BROWSE_WIND_BEARING),
    ...
    std::string cloudCoverNameId = parentNameId + "." + weatherserver::WeatherData::BROWSE_CLOUD_COVER;
    UA_NodeId cloudCoverVarNodeId = UA_NODEID_STRING_ALLOC(weatherserver::WebService::OPC_NS_INDEX,
    ...
    cloudCoverVarAttr.displayName = UA_LOCALIZEDTEXT(locale, weatherserver::WeatherData::BROWSE_CLOUD_COVER);
    ...
    UA_Server_addDataSourceVariableNode(server, cloudCoverVarNodeId, parentNodeId,
        UA_NODEID_NUMERIC(0, UA_NS0ID_HASCOMPONENT),
        UA_QUALIFIEDNAME(weatherserver::WebService::OPC_NS_INDEX, weatherserver::WeatherData::BROWSE_CLOUD_COVER),
...
@param UA_NodeId parentNodeId - The parent node id of the object where the variables will be added in OPC UA.
*/
static void requestLocations(UA_Server* server, weatherserver::CountryData& country, UA_NodeId parentNodeId) {
    try {
        // ReSharper disable once CppExpressionWithoutSideEffects
        webService->fetchAllLocations(country.getCode(), country.getLocationsNumber()).then([&](web::json::value response) {
            country.setLocations(weatherserver::LocationData::parseJsonArray(response));
            ...
            for (size_t i {0}; i < country.getLocations().size(); i++) {
                ...
                std::string countries {weatherserver::CountryData::COUNTRIES_FOLDER_NODE_ID};
                std::string locationObjNameId =
                    static_cast<std::string>(weatherserver::CountryData::COUNTRIES_FOLDER_NODE_ID)
                    + "." + locationCountryCode + "." + locationName;
                /* Creates an Location object node containing all the weather information related to it. */
                UA_NodeId locationObjId = UA_NODEID_STRING_ALLOC(weatherserver::WebService::OPC_NS_INDEX,
                ...
                UA_Server_addObjectNode(server, locationObjId, parentNodeId,
                    ...
                    UA_QUALIFIEDNAME_ALLOC(weatherserver::WebService::OPC_NS_INDEX, locationName.c_str()),
                    UA_NODEID_NUMERIC(0, UA_NS0ID_BASEOBJECTTYPE), locationObjAttr, NULL, NULL);

                std::string flagInitializeVarNameId = locationObjNameId + "." + weatherserver::LocationData::BROWSE_FLAG_INITIALIZE;
                UA_NodeId flagInitializeVarNodeId = UA_NODEID_STRING_ALLOC(weatherserver::WebService::OPC_NS_INDEX, flagInitializeVarNameId.c_str());
                ...
                flagInitializeVarAttr.displayName = UA_LOCALIZEDTEXT(locale, weatherserver::LocationData::BROWSE_FLAG_INITIALIZE);
                UA_Server_addVariableNode(server, flagInitializeVarNodeId, locationObjId,
                    UA_NODEID_NUMERIC(0, UA_NS0ID_HASCOMPONENT),
                    UA_QUALIFIEDNAME(weatherserver::WebService::OPC_NS_INDEX, weatherserver::LocationData::BROWSE_FLAG_INITIALIZE),
...
void requestCountries(UA_Server * server, const UA_NodeId &parentNodeId) {
    try {
        webService->fetchAllCountries().then([&](web::json::value response) {
            webService->setAllCountries(weatherserver::CountryData::parseJsonArray(response));
            ...
            for (size_t i {0}; i < webService->getAllCountries().size(); i++) {
                ...
                std::string countryObjNameId = static_cast<std::string>(weatherserver::CountryData::COUNTRIES_FOLDER_NODE_ID) + "." + countryCode;
                ...
                UA_NodeId countryObjId = UA_NODEID_STRING_ALLOC(weatherserver::WebService::OPC_NS_INDEX, countryObjNameId.c_str());
                ...
                UA_Server_addObjectNode(server, countryObjId, parentNodeId,
                    UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES),
                    UA_QUALIFIEDNAME_ALLOC(weatherserver::WebService::OPC_NS_INDEX, countryName.c_str()),
                    UA_NODEID_NUMERIC(0, UA_NS0ID_FOLDERTYPE), countryObjAttr, NULL, NULL);

                std::string nameVarNameId = countryObjNameId + "." + weatherserver::CountryData::BROWSE_NAME;
                UA_NodeId nameVarNodeId = UA_NODEID_STRING_ALLOC(weatherserver::WebService::OPC_NS_INDEX, nameVarNameId.c_str());
                ...
                nameVarAttr.displayName = UA_LOCALIZEDTEXT(locale, weatherserver::CountryData::BROWSE_NAME);
                UA_Server_addVariableNode(server, nameVarNodeId, countryObjId,
                    UA_NODEID_NUMERIC(0, UA_NS0ID_HASCOMPONENT),
                    UA_QUALIFIEDNAME(weatherserver::WebService::OPC_NS_INDEX, weatherserver::CountryData::BROWSE_NAME),
                    UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE), nameVarAttr, NULL, NULL);

                std::string codeVarNameId = countryObjNameId + "." + weatherserver::CountryData::BROWSE_CODE;
                UA_NodeId codeVarNodeId = UA_NODEID_STRING_ALLOC(weatherserver::WebService::OPC_NS_INDEX, codeVarNameId.c_str());
                ...
                codeVarAttr.displayName = UA_LOCALIZEDTEXT(locale, weatherserver::CountryData::BROWSE_CODE);
                UA_Server_addVariableNode(server, codeVarNodeId, countryObjId,
                    UA_NODEID_NUMERIC(0, UA_NS0ID_HASCOMPONENT),
                    UA_QUALIFIEDNAME(weatherserver::WebService::OPC_NS_INDEX, weatherserver::CountryData::BROWSE_CODE),
                    UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE), codeVarAttr, NULL, NULL);

                std::string citiesNumberVarNameId = countryObjNameId + "." + weatherserver::CountryData::BROWSE_CITIES_NUMBER;
                UA_NodeId citiesNumberVarNodeId = UA_NODEID_STRING_ALLOC(weatherserver::WebService::OPC_NS_INDEX, citiesNumberVarNameId.c_str());
                ...
                citiesNumberVarAttr.displayName = UA_LOCALIZEDTEXT(locale, weatherserver::CountryData::BROWSE_CITIES_NUMBER);
                UA_Server_addVariableNode(server, citiesNumberVarNodeId, countryObjId,
                    UA_NODEID_NUMERIC(0, UA_NS0ID_HASCOMPONENT),
                    UA_QUALIFIEDNAME(1, weatherserver::CountryData::BROWSE_CITIES_NUMBER),
                    UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE), citiesNumberVarAttr, NULL, NULL);

                std::string locationsNumberVarNameId = countryObjNameId + "." + weatherserver::CountryData::BROWSE_LOCATIONS_NUMBER;
                UA_NodeId locationsNumberVarNodeId = UA_NODEID_STRING_ALLOC(weatherserver::WebService::OPC_NS_INDEX,
                ...
                locationsNumberVarAttr.displayName = UA_LOCALIZEDTEXT(locale, weatherserver::CountryData::BROWSE_LOCATIONS_NUMBER);
                UA_Server_addVariableNode(server, locationsNumberVarNodeId, countryObjId,
                    UA_NODEID_NUMERIC(0, UA_NS0ID_HASCOMPONENT),
                    UA_QUALIFIEDNAME(1, weatherserver::CountryData::BROWSE_LOCATIONS_NUMBER),
...
static void addCountries(UA_Server* server) {
    ...
    if (!wasItCalled) {
        ...
        UA_NodeId countriesObjId = UA_NODEID_STRING(weatherserver::WebService::OPC_NS_INDEX,
            weatherserver::CountryData::COUNTRIES_FOLDER_NODE_ID);
        ...
        countriesObjAttr.displayName = UA_LOCALIZEDTEXT(locale, weatherserver::CountryData::COUNTRIES_FOLDER_NODE_ID);
        UA_Server_addObjectNode(server, countriesObjId,
            UA_NODEID_NUMERIC(0, UA_NS0ID_OBJECTSFOLDER),
            UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES),
            UA_QUALIFIEDNAME(weatherserver::WebService::OPC_NS_INDEX, weatherserver::CountryData::COUNTRIES_FOLDER_NODE_ID),
                ...
                if (posDot != std::string::npos) {
                    ...
                    */
                    if (nodeIdName.size() > locationObjNameId.size()) {
                        UA_NodeId locationObjId = UA_NODEID_STRING_ALLOC(weatherserver::WebService::OPC_NS_INDEX, locationObjNameId.c_str());
                        /*
